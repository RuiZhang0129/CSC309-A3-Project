...
app.post("/users/:userId/transactions", authenticate, async (req, res) => {
    try {
        ...
        let recipient2;
        if (rawId === "me") {
            recipient2 = await prisma.user.findUnique({ where: { id: req.user.id } });
        } else if (!isNaN(rawId)) {
            recipient2 = await prisma.user.findUnique({ where: { id: parseInt(rawId, 10) } });
        } else {
            recipient2 = await prisma.user.findUnique({ where: { utorid: rawId } });
        }
