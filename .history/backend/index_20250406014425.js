...
app.post("/users/:userId/transactions", authenticate, async (req, res) => {
    try {
        ...
        if (rawId === "me") {
            recipientId = await prisma.user.findUnique({ where: { id: req.user.id } });
        } else {
        // 优先按 utorid 查
        recipientId = await prisma.user.findUnique({ where: { utorid: rawId } });
        // 如果 utorid 查不到，且是数字，再尝试按 id 查
        if (!recipientId && /^\d+$/.test(rawId)) {
            recipientId = await prisma.user.findUnique({ where: { id: parseInt(rawId, 10) } });
        }
        }
        ...
        const recipientId = recipient.id;  // ✅ 这下 recipientId 就有定义了
        const { type, amount, remark = "" } = req.body;
