...
//   const [type, setType] = useState('');
  ...
  }, [page, order, searchName, published]);
    ...
    <div style={{ padding: '20px' }}>
      ...
      {events.map(event => (
        <div key={event.id} style={{ border: '1px solid #ccc', borderRadius: '6px', padding: '10px', marginBottom: '10px' }}>
          ...
          <p><strong>发布状态:</strong> {events.published ? '✅ 已发布' : '❌ 未发布'}</p>
          <p><strong>积分剩余:</strong> {events.pointsRemain}</p>
          <p><strong>已发放积分:</strong> {events.pointsAwarded}</p>
