...
app.get("/users/:userId/utorid", authenticate, async (req, res) => {
    const userId = parseInt(req.params.userId, 10);
    if (isNaN(userId)) {
    ...
    try {
        ...
        console.error("Error fetching utorid:", err);
        return res.status(500).json({ error: "Internal Server Error" });
    }
});
...
app.post("/transactions", authenticate, async (req, res) => {
    try {
        ...
        if (type === "purchase") {
            ...
            const baseEarned = Math.round(spent * 4); 
            let bonusEarned = 0;
            let usedPromotions = [];
...
app.get("/transactions/:transactionId", authenticate, authorize("manager", "superuser"), async (req, res) => {
    try {
...
app.patch("/transactions/:transactionId/suspicious", authenticate, authorize("manager", "superuser"), async (req, res) => {
    try {
        const transactionId = parseInt(req.params.transactionId, 10);
        if (transactionId === undefined || isNaN(transactionId)) {
            return res.status(400).json({ error: "Invalid transaction ID" });
    ...
    try {
        ...
        const transaction = await prisma.transaction.create({
            data: {
                ...
                remark,
                processed: false,  
                creator: { connect: { utorid: user.utorid } }  
            }
        });
...
app.get("/users/me/redemption-qr", authenticate, async (req, res) => {
    ...
    if (!tx) return res.status(404).json({ error: "No unprocessed redemption request found" });
    return res.json(tx);
});

app.get("/users/me/redemptions", authenticate, async (req, res) => {
...
app.post("/users/:userId/transactions", authenticate, async (req, res) => {
    try {
        ...
        if (rawId === "me") {
            recipient = await prisma.user.findUnique({ where: { id: req.user.id } });
        } else {
            recipient = await prisma.user.findUnique({ where: { utorid: rawId } });
            if (!recipient && /^\d+$/.test(rawId)) {
                recipient = await prisma.user.findUnique({ where: { id: parseInt(rawId, 10) } });
            }
        }
...
app.get("/events", authenticate, async (req, res) => {
    try {
        ...
        const organizedOnly = req.query.organizedOnly === "true";
        if (organizedOnly) {
            where.OR = [
                { organizerId: req.user.id },
                { organizers: { some: { id: req.user.id } } }
            ];
        }
        ...
        const guestCounts = await Promise.all(results.map(e =>
            prisma.eventParticipants.count({ where: { eventId: e.id } })
        ));

        const withGuestCounts = results.map((e, i) => ({
            ...e,
            numGuests: guestCounts[i]
        }));



        return res.status(200).json({ count, results: withGuestCounts });
    } catch (err) {
